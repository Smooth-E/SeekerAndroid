...
namespace Seeker
{
    public class TransfersFragment : Fragment, PopupMenu.IOnMenuItemClickListener
    {
        ...
        public override bool OnOptionsItemSelected(IMenuItem item)
        {
            switch (item.ItemId)
            {
                ...
                case Resource.Id.action_clear_all_complete: //clear all complete
                    Logger.FirebaseInfo("Clear All Complete Pressed");
                    if (CurrentlySelectedDLFolder == null)
                    {
                ...
                case Resource.Id.action_cancel_and_clear_all: //cancel and clear all
                    Logger.FirebaseInfo("action_cancel_and_clear_all Pressed");
                ...
                case Resource.Id.action_abort_all: //abort all
                    Logger.FirebaseInfo("action_abort_all_pressed");
                ...
                case Resource.Id.action_pause_all:
                    Logger.FirebaseInfo("pause all Pressed");
                    SeekerState.CancelAndClearAllWasPressedDebouncer = DateTimeOffset.Now.ToUnixTimeMilliseconds();
                    if (CurrentlySelectedDLFolder == null)
                    {
        ...
        public override View OnCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState)
        {
            ...
            Logger.FirebaseInfo("AutoClear: " + SeekerState.AutoClearCompleteDownloads);
            Logger.FirebaseInfo("AutoRetry: " + SeekerState.AutoRetryDownload);
        ...
        private void DownloadRetryLogic(ITransferItem transferItem)
        {
            ...
            if (SeekerState.SoulseekClient.IsTransferInDownloads(item1.Username, item1.FullFilename/*, out tokenNum*/))
            {
                ...
                {
                    Logger.FirebaseDebug("CTS is null. this should not happen. we should always set it before downloading.");
                }
        ...
        public override bool OnContextItemSelected(IMenuItem item)
        {
            if (item.GroupId == UNIQUE_TRANSFER_GROUP_ID)
            {
                ...
                {
                    Logger.FirebaseInfo("recyclerTransferAdapter is null");
                }
                ...
                if (ti == null)
                {
                    Logger.FirebaseInfo("ti is null");
                }
                ...
                switch (item.ItemId)
                {
                    ...
                    case 1:
                        ...
                        Logger.FirebaseInfo("Clear Complete item pressed");
                        lock (TransferItemManagerWrapped.GetUICurrentList()) //TODO: test
                        {
                            ...
                            {
                                //Logger.FirebaseDebug("case1: info.Position: " + position + " transferItems.Count is: " + transferItems.Count);
                                Toast.MakeText(SeekerState.ActiveActivityRef, "Selected transfer does not exist anymore.. try again.", ToastLength.Short).Show();
                                return base.OnContextItemSelected(item);
                            }
                    ...
                    case 2: //cancel and clear (downloads) OR abort and clear (uploads)
                        //info = (AdapterView.AdapterContextMenuInfo)item.MenuInfo;
                        Logger.FirebaseInfo("Cancel and Clear item pressed");
                        ...
                        {
                            //Logger.FirebaseDebug("case2: info.Position: " + position + " transferItems.Count is: " + transferItems.Count);
                            Toast.MakeText(SeekerState.ActiveActivityRef, "Selected transfer does not exist anymore.. try again.", ToastLength.Short).Show();
                            return base.OnContextItemSelected(item);
                        }
                        ...
                        {
                            Logger.FirebaseInfo("Cancel and Clear item pressed - bad item");
                        }
                        break;
                    case 3:
                        ...
                        {
                            //Logger.FirebaseDebug("case3: info.Position: " + position + " transferItems.Count is: " + transferItems.Count);
                            Toast.MakeText(SeekerState.ActiveActivityRef, "Selected transfer does not exist anymore.. try again.", ToastLength.Short).Show();
                            return base.OnContextItemSelected(item);
                        }
                    ...
                    case 4:
                        ...
                        {
                            //Logger.FirebaseDebug("case4: info.Position: " + position + " transferItems.Count is: " + transferItems.Count);
                            Toast.MakeText(SeekerState.ActiveActivityRef, "Selected transfer does not exist anymore.. try again.", ToastLength.Short).Show();
                            return base.OnContextItemSelected(item);
                        }
                        try
                    ...
                    case 103: //abort upload
                        Logger.FirebaseInfo("Abort Upload item pressed");
                        tItem = null;
                        try
                        {
                            tItem = TransferItemManagerWrapped.GetItemAtUserIndex(position);
                    ...
                    case 104: //ignore (unshare) user
                        Logger.FirebaseInfo("Unshare User item pressed");
            ...
            public bool OnActionItemClicked(ActionMode mode, IMenuItem item)
            {
                switch (item.ItemId)
                {
                    ...
                    case Resource.Id.action_cancel_and_clear_all_batch:
                        Logger.FirebaseInfo("action_cancel_and_clear_batch Pressed");
                        SeekerState.CancelAndClearAllWasPressedDebouncer = DateTimeOffset.Now.ToUnixTimeMilliseconds();
        ...
        public void GetQueuePosition(TransferItem ttItem)
        {
            ...
            Func<TransferItem, object> actionOnComplete = new Func<TransferItem, object>((TransferItem t) =>
            {
                ...
                {
                    Logger.FirebaseDebug("actionOnComplete" + e.Message + e.StackTrace);
                }
...
#pragma warning restore 0618
        ...
        private void refreshListViewSpecificItem(int indexOfItem)
        {
            ...
            if (Context == null)
            {
                ...
                {
                    Logger.FirebaseDebug("cannot refreshListView on TransferStateUpdated, MainActivityRef and Context are null");
                    return;
                }
            ...
            {
                Logger.FirebaseDebug("cannot refreshListView on TransferStateUpdated, noTransfers is null");
                return;
            }
            SetNoTransfersMessage();
        ...
        public abstract class TransferAdapterRecyclerVersion : RecyclerView.Adapter //<TransferAdapterRecyclerVersion.TransferViewHolder>
        {
            ...
            public void setSelectedItem(ITransferItem item)
            {
                ...
                {
                    Logger.FirebaseInfo("selected item was set as null");
                }
            }
