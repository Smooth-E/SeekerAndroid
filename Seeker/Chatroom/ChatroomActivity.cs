...
namespace Seeker
{
    ...
    public class ChatroomActivity : SlskLinkMenuActivity//, Android.Widget.PopupMenu.IOnMenuItemClickListener
    {
        ...
        protected override void OnCreate(Bundle savedInstanceState)
        {
            ...
            else if (Intent != null) //if an intent started this activity
            {
                if (Intent.GetBooleanExtra(ChatroomController.ComingFromMessageTapped, false))
                {
                    ...
                    {
                        Logger.FirebaseDebug("empty goToUsersMessages");
                    }
                    else
                    {
                        startWithUserFragment = true;
        ...
        protected override void OnNewIntent(Intent intent)
        {
            ...
            if (intent.GetBooleanExtra(ChatroomController.ComingFromMessageTapped, false))
            {
                ...
                if (goToRoom == string.Empty)
                {
                    Logger.FirebaseDebug("empty goToRoom");
                }
                else
                {
        ...
        private static void RestoreStartingRoomInfo(Bundle inState)
        {
            ...
            {
                Logger.FirebaseDebug("no restore info...");
                return;
            }
            Logger.FirebaseDebug("restoring info...");
        ...
        {
            Logger.FirebaseInfo("ShowAllTickersDialog" + this.IsFinishing + this.IsDestroyed + SupportFragmentManager.IsDestroyed);
        ...
        {
            Logger.FirebaseInfo("ShowInviteUserDialog" + this.IsFinishing + this.IsDestroyed);
            ...
            catch (WindowManagerBadTokenException e)
            {
                ...
                {
                    Logger.FirebaseDebug("invite WindowManagerBadTokenException null activities");
                }
                else
                {
                    bool isCachedMainActivityFinishing = SeekerState.ActiveActivityRef.IsFinishing;
                    bool isOurActivityFinishing = this.IsFinishing;
                    Logger.FirebaseDebug("invite WindowManagerBadTokenException are we finishing:" + isCachedMainActivityFinishing + isOurActivityFinishing);
                }
            }
            catch (Exception err)
            {
                if (SeekerState.ActiveActivityRef == null)
                {
                    Logger.FirebaseDebug("invite Exception null activities");
                }
                else
                {
                    bool isCachedMainActivityFinishing = SeekerState.ActiveActivityRef.IsFinishing;
                    bool isOurActivityFinishing = this.IsFinishing;
                    Logger.FirebaseDebug("invite Exception are we finishing:" + isCachedMainActivityFinishing + isOurActivityFinishing);
                }
            }
        ...
        {
            Logger.FirebaseInfo("ShowSetTickerDialog" + this.IsFinishing + this.IsDestroyed);
        ...
        {
            Logger.FirebaseInfo("ShowEditCreateChatroomDialog" + this.IsFinishing + this.IsDestroyed);
            //AndroidX.AppCompat.App.AlertDialog.Builder builder = new AndroidX.AppCompat.App.AlertDialog.Builder(); //failed to bind....
            Context c = this;
            ...
            catch (WindowManagerBadTokenException e)
            {
                if (SeekerState.ActiveActivityRef == null)
                {
                    Logger.FirebaseDebug("createroomWindowManagerBadTokenException null activities");
                }
                else
                {
                    bool isCachedMainActivityFinishing = SeekerState.ActiveActivityRef.IsFinishing;
                    bool isOurActivityFinishing = this.IsFinishing;
                    Logger.FirebaseDebug("createroomWindowManagerBadTokenException are we finishing:" + isCachedMainActivityFinishing + isOurActivityFinishing);
                }
            }
            catch (Exception err)
            {
                if (SeekerState.ActiveActivityRef == null)
                {
                    Logger.FirebaseDebug("createroomException null activities");
                }
                else
                {
                    bool isCachedMainActivityFinishing = SeekerState.ActiveActivityRef.IsFinishing;
                    bool isOurActivityFinishing = this.IsFinishing;
                    Logger.FirebaseDebug("createroomException are we finishing:" + isCachedMainActivityFinishing + isOurActivityFinishing);
                }
            }
        ...
        private void Input_FocusChange(object sender, View.FocusChangeEventArgs e)
        {
            ...
            {
                Logger.FirebaseDebug("createroomMainActivity_FocusChange" + err.Message);
            }
        }
    ...
    public class GroupMessageInnerViewReceivedHolder : RecyclerView.ViewHolder, View.IOnCreateContextMenuListener
    {
        public void OnCreateContextMenu(IContextMenu menu, View v, IContextMenuContextMenuInfo menuInfo)
        {
            ...
            {
                Logger.FirebaseDebug("sender for GroupMessageInnerViewReceivedHolder.GroupMessageInnerViewReceived " 
                                     + "is " + v.GetType().Name);
            }
